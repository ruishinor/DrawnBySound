...
export const SETTINGS_STORAGE_KEY = 'vibratoflow.settings.v2';
...
export class SettingsStore {
  ...
  /** Reset visual tuning while preserving the user's last chosen source. */
  reset(): Readonly<Settings> {
    const { preferredSource } = this.settings;
    this.settings = { ...DEFAULT_SETTINGS, preferredSource };
    this.save();
    return this.settings;
  }
